...
#ifdef EXP_TILES
void blfilter_Read_Tiles(float *paddedImg, float *gaussian_sp, int outerTileSize, int outerTileSize, int rows, int cols, int filter_hw, float sigma_sp, float sigma_ph, float *Op_Img);
#endif
#ifdef USE_TILES
...
int main(int argc, char* argv[])
{
    ...
    if (argc != 7)
    {
        fprintf(stderr, "Usage: %s <Image File> <Filter Radius> <Sigma Spatial> <Sigma Photometric> <outerTileSize> <innerTileSize>\n", argv[0]);
        exit(0);
    }
    int outerTileSize = atoi(argv[5]);
    int innerTileSize = atoi(argv[6]);
#else
    if (argc != 5)
    {
...
#if defined USE_TILES || EXP_TILES
    if ((cols % outerTileSize != 0) || (rows % outerTileSize !=0) ||(outerTileSize % innerTileSize !=0))
    {
        printf("Image width = %d or Height = %d is not divisible by TileSize %d or smallerTile %d\nexiting..",cols, rows, outerTileSize, innerTileSize);
        exit(1);
    }
#endif

    /* Check for erros */
    posix_memalign((void **)&Ip_Img, 64, rows * cols * sizeof(float));
    posix_memalign((void **)&Op_Img, 64, rows * cols * sizeof(float));
    ...
    posix_memalign((void **)&gaussian_sp, 64, ( (2 * filter_hw + 1) *  (2 * filter_hw + 1) * sizeof(float)));
    ...
    posix_memalign((void **)&paddedImg, 64, ( (rows + 2 * filter_hw) * (cols + 2 * filter_hw) * sizeof(float)));
    ...
    start = my_difftime();
#ifdef USE_TILES
    blfilter_Tiles(paddedImg, gaussian_sp, outerTileSize, rows, cols, filter_hw, sigma_sp, sigma_ph, Op_Img);
#elif EXP_TILES
    blfilter_Read_Tiles(paddedImg, gaussian_sp, outerTileSize, innerTileSize, rows, cols, filter_hw, sigma_sp, sigma_ph, Op_Img);
#else
...
void blfilter_Tiles(float *paddedImg, float *gaussian_sp, int TileSize, int rows, int cols, int filter_hw, float sigma_sp, float sigma_ph, float *Op_Img)
{
    ...
    posix_memalign((void **)&Ip_Tile, 64, ( (TileSize + 2 * filter_hw ) * ( TileSize + 2 * filter_hw) * sizeof(float),16));
    posix_memalign((void **)&Op_Tile, 64, ( TileSize  * TileSize  * sizeof(float), 16));
    float sigmasq  = 1/(-2*sigma_ph*sigma_ph);
#ifdef USE_OMP
...
/* Funtion to Apply Bilateral filtering using Tiles explicitly reading tiles
* ----------------------------------------------------------------------*
* ----------------------------------------------------------------------*
* ----------------------------------------------------------------------*
* ----------------------------------------------------------------------*/
...
#ifdef EXP_TILES
void blfilter_Read_Tiles(float *paddedImg, float *gaussian_sp, int outerTileSize, int innerTileSize, int rows, int cols, int filter_hw, float sigma_sp, float sigma_ph, float *Op_Img)
{
    float filter_current_pixel = 0; //local window
    float pd = 0, pixel_value=0;          //photometric distance
    float gaussian_ph = 0; //photometric filter coefficient
    ...
    float *Ip_Tile, *Op_Tile;
    float *inner_Ip_Tile, *inner_Op_Tile;
...
#pragma omp for private( tiles, filter_current_pixel,i,j,k,l,innertiles,normal_factor,gaussian_ph, gaussian_bl, filtered_pixel,pd)
        ...
        for( tiles = 0; tiles < numTiles; tiles++)
        {
            ...
            for (innertiles = 0; innertiles < innernumTiles; innertiles++)
            {
                ...
                for( i = filter_hw; i < innerTileSize + filter_hw ; i++)
                {
                ...
                WriteTile(inner_Op_Tile, outerTileSize, innerTileSize,Op_Tile, innertiles);
            }
            WriteTile(Op_Tile, Cols, outerTileSize,Op_Img, tiles);
        }
#ifdef USE_OMP
    }
#endif
